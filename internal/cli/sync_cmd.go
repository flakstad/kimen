...
func validatePushBaseline(lastSeen, remoteRev string, hasRemote bool) error {
	...
	case !hasRemote && lastSeen == "":
		return nil
	case !hasRemote && lastSeen != "":
		return fmt.Errorf("%w: remote bundle disappeared since last sync (expected rev %s)", errSyncConflict, lastSeen)
	case hasRemote && lastSeen == "":
		return fmt.Errorf("%w: remote has data (rev %s) but no local baseline; run `kimen sync pull` first", errSyncConflict, remoteRev)
	case hasRemote && lastSeen != remoteRev:
		return fmt.Errorf("%w: remote changed (expected rev %s, found %s); run `kimen sync pull`, re-apply changes, then push", errSyncConflict, lastSeen, remoteRev)
	default:
		return nil
	}
}
