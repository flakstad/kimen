...
// - $HOME/.config/kimen/profiles/<name>.kmap
...
func ResolveProfile(name string) (string, error) {
	...
	if dir := strings.TrimSpace(os.Getenv(envProfileDir)); dir != "" {
		candidates = append(candidates, filepath.Join(dir, filename))
	}
	candidates = append(candidates, filepath.Join(".kimen", "profiles", filename))
	if xdgConfigHome := strings.TrimSpace(os.Getenv("XDG_CONFIG_HOME")); xdgConfigHome != "" {
		candidates = append(candidates, filepath.Join(xdgConfigHome, "kimen", "profiles", filename))
	}
	if homeDir, err := os.UserHomeDir(); err == nil && strings.TrimSpace(homeDir) != "" {
		candidates = append(candidates, filepath.Join(homeDir, ".config", "kimen", "profiles", filename))
	}
