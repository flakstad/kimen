...
(defn- handle-completion
  ...
  (let [args (vec args)]
    (cond
      (empty? args)
      (error-with-help 1 "missing required argument <shell> (expected one of: bash, zsh, fish, powershell)" completion-usage)
      ...
      (let [shell (-> (first args) str str/trim str/lower-case)
            script (completion-script shell)]
        (if (nil? script)
          (error-with-help 1 (str "unsupported shell " (pr-str shell) " (expected one of: bash, zsh, fish, powershell)")
                           completion-usage)
          (result {:exit-code 0
                   :stdout script})))))) 
